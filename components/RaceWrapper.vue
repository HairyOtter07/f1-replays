...
const getInitialTime = () => {
    const { data } = useLazyAsyncData("heartbeat", async () => {
        const res = await $fetch(
            `/api/${race.split("-")[0]}/${race}/${race.split("_")[0]}_Race/Heartbeat.jsonStream`,
            {
                responseType: "text",
            },
        );
...
const fetchEndpoint = (endpoint) => {
    const { data, status, error } = useLazyAsyncData(endpoint, async () => {
        const res = await $fetch(
            `/api/${race.split("-")[0]}/${race}/${race.split("_")[0]}_Race/${endpoint}`,
            {
                responseType: "text",
            },
        );
...
const fetchAllEndpoints = (endpoints) => {
    ...
    if (endpoints.length > 1) {
        ...
        reactToRef(fetchedData, () => {
            ...
            reactToRef(rest, () => {
                const combined = [
                    { endpoint, data: fetchedData.value },
                    ...rest.value,
                ];
                data.value = combined;
                status.value = true;
...
const processData = async (dataArray) => {
    ...
    const endTime = sessionStatus.find(
        (e) => e.data.Status == "Ends",
    ).timestamp;
    ...
    while (
        ...
        if (
            ...
            for (const driverKey of Object.keys(pos.data)) {
                ...
                if (
                    Object.keys(
                        timingData[leaderboardIndex].data.Lines,
                    ).includes(driverKey) &&
                    Object.keys(
